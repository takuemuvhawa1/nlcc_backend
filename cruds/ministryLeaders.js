...
ministryLeadersObj.postLeader = (MinistryID, LeaderID, StartDate, EndDate) => {
    return new Promise((resolve, reject) => {
        pool.query('INSERT INTO ministry_leaders(MinistryID, LeaderID, StartDate, EndDate) VALUES (?, ?, ?, ?)', 
...
ministryLeadersObj.getLeaders = () => {
    return new Promise((resolve, reject) => {
        pool.query('SELECT * FROM ministry_leaders', (err, results) => {
            if (err) return reject(err);
...
ministryLeadersObj.getLeaderById = (leaderId) => {
    return new Promise((resolve, reject) => {
        pool.query('SELECT * FROM ministry_leaders WHERE MinistryLeaderID = ?', [leaderId], (err, results) => {
            if (err) return reject(err);
            return resolve(results);
        });
    });
...
ministryLeadersObj.updateLeader = (MinistryLeaderID, MinistryID, LeaderID, StartDate, EndDate) => {
    return new Promise((resolve, reject) => {
        pool.query('UPDATE ministry_leaders SET MinistryID = ?, LeaderID = ?, StartDate = ?, EndDate = ? WHERE MinistryLeaderID = ?',
...
ministryLeadersObj.deleteLeader = (leaderId) => {
    return new Promise((resolve, reject) => {
        pool.query('DELETE FROM ministry_leaders WHERE MinistryLeaderID = ?', [leaderId], (err, results) => {
            if (err) return reject(err);
