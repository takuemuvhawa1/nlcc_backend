...
notificationsRouter.post('/', async (req, res) => {
    try {
        ...
        const results = await notificationsDbOperations.postNotification(postedValues.type, postedValues.theme, postedValues.date, postedValues.time);
        res.json(results);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
...
notificationsRouter.put('/:id', async (req, res) => {
    try {
        ...
        const result = await notificationsDbOperations.updateNotification(id, updatedValues.type, updatedValues.theme, updatedValues.date, updatedValues.time);
        res.json(result);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
